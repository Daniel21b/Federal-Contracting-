...
async function loadStateData(state) {
  ...
  // Try loading the data with different date strategies
  const container = d3.select("#chart");
  
  // Strategy 1: Try with current date
  const currentDate = new Date().toISOString().split('T')[0];
  let fileName = `../processed_data/${state}_contracts_${currentDate}.csv`;
  
  try {
    // First attempt with current date
    await loadAndProcessData(fileName, container, state);
  } catch (error) {
    console.log(`Could not load file with current date (${currentDate}), trying yesterday's date...`);
    
    // Strategy 2: Try with yesterday's date
    const yesterday = new Date();
    yesterday.setDate(yesterday.getDate() - 1);
    const yesterdayStr = yesterday.toISOString().split('T')[0];
    fileName = `../processed_data/${state}_contracts_${yesterdayStr}.csv`;
    
    try {
      await loadAndProcessData(fileName, container, state);
    } catch (error) {
      console.log(`Could not load file with yesterday's date (${yesterdayStr}), trying fallback...`);
      
      // Strategy 3: Try with hardcoded fallback date
      fileName = `../processed_data/${state}_contracts_2025-05-18.csv`;
      try {
        await loadAndProcessData(fileName, container, state);
      } catch (finalError) {
        console.error(`Error loading ${state} data:`, finalError);
        container.append("p")
          .attr("class", "error")
          .style("color", "red")
          .style("text-align", "center")
          .text(`Error loading ${state} data. Please check available data files.`);
      }
    }
  }
}
...
async function loadAndProcessData(fileName, container, state) {
  ...
  await createChart(keyframes, nameframes, prev, next, container);
}

// Create chart function (moved from loadStateData)
async function createChart(keyframes, nameframes, prev, next, container) {
...
function ticker(svg, keyframes) {
  const now = svg.append("text")
    ...
    .attr("text-anchor", "end")
    .attr("x", width - 6)
    .attr("y", marginTop + barSize * (n - 0.2))
    .attr("dy", "0.32em")
